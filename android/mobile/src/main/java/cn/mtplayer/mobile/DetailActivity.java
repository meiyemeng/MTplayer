...
public final class DetailActivity extends AppCompatActivity {
    ...
    private MediaItem item; private MediaDetail detail; private LinearLayout root; private LocalLibrary library;
    ...
    private void load(){executor.execute(()->{try{List<Site> sites=AppServices.configurations.enabledSites();Site selected=null;for(Site site:sites)if(site.key.equals(item.siteKey)){selected=site;break;}if(selected==null)throw new IllegalStateException("该播放接口当前未启用");detail=AppServices.catalogue.detail(selected,item.id);runOnUiThread(this::render);}catch(Exception ex){runOnUiThread(()->{root.addView(Ui.text(this,"加载失败："+(ex.getMessage()==null?"未知错误":ex.getMessage())),Ui.matchWrap());});}});}
    private void render(){
        ...
        play.setOnClickListener(v->{if(selected[0]==null)return;library.record(item);Intent i=new Intent(this,PlayerActivity.class);i.putExtra("url",selected[0].url);i.putExtra("title",detail.item.name+" · "+selected[0].name);i.putExtra("mediaKey",item.siteKey+":"+item.id);startActivity(i);});
    }
